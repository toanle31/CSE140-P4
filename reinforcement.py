...
class ReinforcementAgent(ValueEstimationAgent):
    ...
    def observationFunction(self, state):
        ...
        if self.lastState is not None:
            ...
            if len(state.getFood().asList()) == len(self.lastState.getFood().asList()):
                reward -= len(state.getFood().asList())
            else:
                reward += len(state.getFood().asList())
            # foodList = state.getFood().asList()
            # distanceToFood = min([self.getMazeDistance(state.getPosition(), food) for food in foodList])
            # reward += 20 / (distanceToFood + 1)
